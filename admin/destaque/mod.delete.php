<?php
 ...
 $sql_guarda = "SELECT ${var['pre']}_titulo,${var['pre']}_imagem,${var['pre']}_video  FROM ".TABLE_PREFIX."_${var['path']} WHERE ${var['pre']}_id=?";
 ...
 $qry_guarda->bind_result($nome, $arq, $video); 
 $qry_guarda->fetch(); 
 $qry_guarda->close();
...
*/
	...
	$var['path_video']   = PATH_IMG.'/'.$var['path'].'/video';
		...
		if ($qry_rem->execute()) {
		  echo "<b>${nome}</b> removido com êxito!<br>";
			...
			if (!empty($folder[$j]) && is_file($arquivo)) {
		     ...
		     $folder = explode(',',$var['path_video']);
		     for($j=0;$j<count($folder);$j++) {
		      $arquivo = $folder[$j].'/'.$video;
