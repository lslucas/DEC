...
*/
	echo $num;
     ...
     $sql_rem = "UPDATE ".TABLE_PREFIX."_${res['prefix']} SET ${res['pre']}_${res['col']}=NULL WHERE ${res['pre']}_id=?";
	 ...
	 for($i=0;$i<count($row['id']);$i++) { 
      ...
      if($apagado==1)
       echo "video apagado!<br>";
       elseif($apagado>1) echo $apagado." vídeos apagados!<br>";


      if($nao_apagado==1)
       echo "vídeo <b>não</b> já não existe!<br>";
       elseif($nao_apagado>1) echo $nao_apagado." vídeo já não existe!<br>";
   ...
   echo "Não foi possível remover o vídeo!<br>";
 }
