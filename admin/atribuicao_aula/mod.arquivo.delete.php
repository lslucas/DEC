       ...
       if ( !($qry_rem = $conn->prepare($sql_rem)) ) {
         ...
         for($i=0;$i<count($row['id']);$i++) { 
          ...
          if ($qry_rem) {
            ...
            if($apagado==1)
             echo "arquivo apagado!<br>";
             elseif($apagado>1) echo $apagado." imagens apagados!<br>";


            if($nao_apagado==1)
             echo "arquivo <b>não</b> já não existe!<br>";
             elseif($nao_apagado>1) echo $nao_apagado." arquivos já não existem!<br>";
