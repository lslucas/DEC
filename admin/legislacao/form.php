  <div class='error container-error'><div class='error-icon'>
	...
	<ol> 
		<li><label for="download" class="error-validate">Envie algum arquivo</label></li>
		<li><label for="link" class="error-validate">Informe uma link válido</label></li>
...
<p class='header'>Todos os campos com <b>- * -</b> são obrigatórios.</p>
	...
	<li>	
	  <label>Arquivo<span class='small'><a href='javascript:void(0);' class='addImagem' id='min'>adicionar + arquivos</a></span></label>
	  <?php
		  
	    if ($act=='update') {
				  
		    $sql_gal = "SELECT rla_id, rla_arquivo, rla_pos FROM ".TABLE_PREFIX."_r_${var['pre']}_arquivo WHERE rla_${var['pre']}_id=? AND rla_arquivo IS NOT NULL ORDER BY rla_pos ASC;"; 
	  ...
	  ?>
		<tr id="<?=$r_id?>">
		  ...
		  <td class='small'>
		    [<a href='?p=<?=$p?>&delete_imagem&item=<?=$r_id?>&prefix=r_<?=$var['pre']?>_arquivo&pre=rla&col=arquivo&folder=<?=$var['arquivo_folderlist']?>&noVisual' title="Clique para remover o ítem selecionado" class='tip trash-imagem' style="cursor:pointer;" id="<?=$r_id?>">remover</a>]
		  </td>
	...
	<li>
	  <label>Link<span class='small'>Caso seja link externo certifique-se de colocar http://</span></label>
	  <input type='text' name='link' id='link' class='' value='<?=$val['link']?>'>
	</li>
