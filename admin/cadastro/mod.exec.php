...
# include de mensagens do arquivo atual
 ...
 $sql_valida = "SELECT ${var['pre']}_nome,${var['pre']}_email FROM ".TABLE_PREFIX."_${var['table']} WHERE ${var['pre']}_email=?";
 ...
 $qry_valida->close();
   ...
   $res['senha_md5']=md5($res['senha']);
   $tipo = 'user';


   if ($act=='insert') {

     $sql= "INSERT INTO ".TABLE_PREFIX."_${var['table']} 
     ...
     $qry->bind_param('ssss',$res['email'],$res['nome'], $tipo, $res['senha_md5']); 
     $qry->execute();
     $qry->close();


     $sql_id = "SELECT ${var['pre']}_id item FROM ".TABLE_PREFIX."_${var['table']} WHERE ${var['pre']}_email=\"".$res['email']."\"";
     ...
     $qry=$conn->prepare($sql);
     $qry->bind_param('sssi', $res['email'], $tipo, $res['nome'],$res['item']); 
     $qry->execute();
     $qry->close();
